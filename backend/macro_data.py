...
def get_sp500_data():
    ...
    if os.path.exists(SP500_CSV_PATH) and os.path.getsize(SP500_CSV_PATH) > 0:
        try:
            sp500_df = pd.read_csv(SP500_CSV_PATH, index_col=0)
...
def get_sp500_crypto_correlation():
    try:
        ...
        if not os.path.exists(SP500_CSV_PATH):
            get_sp500_data() # Fetch if it doesn't exist
        sp500_df = pd.read_csv(SP500_CSV_PATH, index_col=0)
        sp500_df = clean_historical_dataframe(sp500_df)
        ...
        end_date = min(sp500_df.index.max(), btc_df.index.max())
        ...
        btc_df_daily = btc_df['close'].resample('D').last().reindex(full_date_range) # No ffill for crypto as it's 24/7
